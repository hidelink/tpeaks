...
export default async function AdminTeamsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Clubes</h1>
        <p className="text-sm text-zinc-500">{teams.length} en total</p>
      </div>

      <ul className="divide-y divide-zinc-200">
        {teams.map((team) => {
          // El admin del club si existe; si no, cualquiera del staff.
          const responsable =
            team.memberships.find((m) => m.role === "ADMIN") ??

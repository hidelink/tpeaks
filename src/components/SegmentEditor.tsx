...
export function SegmentEditor({
  ...
  const options = paces && meta.suggestsPaces ? paceOptions(paces) : null;

  /**
   * Un campo se esconde si no aplica al deporte, PERO nunca si ya tiene valor:
   * cambiar de "correr" a "fuerza" no debe ocultar en silencio una distancia
   * que sigue guardada. Se sigue viendo, y el coach decide si la borra.
   */
  const showsDistance = (segment: WorkoutSegment) =>
    meta.usesDistance || segment.distanceMeters !== undefined;
  const showsPace = (segment: WorkoutSegment) => meta.usesPace || segment.targetPace !== undefined;
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs text-zinc-500">
        {meta.usesDistance || meta.usesPace
          ? "Distancia, duración y ritmo son independientes: llena solo lo que aplique a cada segmento (ej. un fondo solo necesita duración; unas series solo distancia y ritmo)."
          : "En este tipo de sesión no hay distancia ni ritmo: usa la etiqueta para el ejercicio (series, repeticiones y peso), la duración si aplica, y el RPE para la intensidad."}
      </p>

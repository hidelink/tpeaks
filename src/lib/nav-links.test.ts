...
describe("navLinksFor", () => {
  it("el dueño ve todo el área de club", () => {
    expect(hrefs("OWNER")).toEqual(STAFF_NAV_LINKS.map((l) => l.href));
  });

  it("administración no ve Plantillas, porque no puede prescribir", () => {
    expect(hrefs("ADMIN")).not.toContain("/coach/templates");
    expect(hrefs("ADMIN")).toContain("/coach/athletes");
    expect(hrefs("ADMIN")).toContain("/coach/settings");
  });
